...
def three_layer_convnet(X,model,y=None, reg=0.0):
    ...
    conv_filter2_height, conv_filter2_width = W1.shape[2:]
...
def init_three_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(3,32,32), num_classes = 10,num_filters=32,filter_size=5):
  ...
  model['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
  model['b1'] = bias_scale * np.random.randn(num_filters)
  model['W2'] = weight_scale * np.random.randn(num_filters * 2, num_filters, filter_size, filter_size)
  model['b2'] = bias_scale * np.random.randn(num_filters* 2)
  model['W3'] = weight_scale * np.random.randn(num_filters * 2 * H * W / 16, num_classes)
